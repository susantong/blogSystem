import {$x, getCss, setAssign} from './common.js'
...
let cutImg = function (dom) {
	...
	const cutImg = $x('.cutImg').get(0);
	const mask = $x('.mask').get(0);
	//preview.style.clip = 'rect(20px 120px 120px 20px)';
	//let width = (document.body.scrollWidth > window.clientWidth) ? document.body.scrollWidth : window.clientWidth;
	//let height = (document.body.scrollHeight > window.clientHeight) ? document.body.scrollHeight : window.clientHeight;
	cutImg.style.width = document.body.scrollWidth  + 'px';
	cutImg.style.height = document.body.scrollHeight  + 'px';
	cutImg.style.left = 0;
	cutImg.style.top = '-80px';
	...
	imgObj.onmousedown = function (e) {
		...
		//console.log(getCss(oTailer, 'left'));
		//console.log(getCss(oTailer, 'top'));
		...
		document.onmousemove = function (event) {
			...
			//console.log(getCss(oTailer, 'height'));
...
/*  裁剪区域的移动  */
	function tailorMove(ev ,oTailor , oPicture ,oShadow) {
	    ...
	    document.onmousemove = function(ev) {
	        var oEvent = ev || window.event;                            
	     	var oLeft = oEvent.clientX - oTmpx;
	     	var oTop = oEvent.clientY - oTmpy;    
